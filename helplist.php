...
<head>
    <title>tech itsolution Academy & J.R KNOWLEDGE INSTITUTE</title>
    <meta charset="utf-8">
...
<body>
    ...
    <div>
        ...
        //     echo '<p class="message"> '.$_SESSION['order_message'].'</p>';
        ...
        <table class="table">
            <thead>
                <tr>
                    ...
                    <th>email_address</th>
                    <th>mobile_number</th>
                    <th>helper_address</th>
                    <th>share_location</th>
                    <th>quantity</th>
                    <th>donated_location</th>
                    <th>view</th>
                    <th>Sumbit_date</th>
            ...
            <tbody>
                <?php 
                     require_once 'C:\xampp\htdocs\Food_world\db_conn.php';
                    // session_start();
                   ...
                   //$phone = $_SESSION['phone'];
                    ...
                    $select_customer_order = "SELECT * from `food_world_database`.`help_table` 
                     order by `submit_date` and `help_id` desc" ;
                    //or 'email_address' = '$email'
                    $result = mysqli_query($conn,$select_customer_order);
                    // print_r($result);
                    //        exit('hum');
                    ...
                    if($num > 1){
                        ...
                        echo" <tr>
                            <th>$sno</th>
                            <th>".$row['help_id']."</th>
                            <th>".$row['helper_name']."</th>
                            <th>".$row['email_address']."</th>
                            <th>".$row['mobile_number']."</th>
                            <th>".$row['helper_address']."</th>
                            <th>".$row['share_location']."</th>
                            <th>".$row['quantity']."</th>
                            <th>".$row['donation_location']."</th>
                            <th>".$row['view']."</th>
                            <th>".$row['submit_date']."</th>
                            <th><a  style='text-decoration:none; text-align:center; background-color:green; 
                            border:2px groove pink; border-radius:5px; height:50px;width:150px;' href=''>
                            <input type='button' value='Edit'></a></th>
                            <th><a href=''><input type='button' value='Delete'></a></th>
                        </tr>";
                        $sno++;
